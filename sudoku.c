...
/*
  Función DFS(Node* initial, int* cont) :
  Esta función recibe el nodo inicial y un contador de iteraciones, las cuales
  son las necesarias para encontrar la solución, siguiendo las instrucciones dadas,
  el código verifica las posibilidades válidas para un sudoku y si es una final, la retorna.
  En caso de no haber encontrado final, se retorna NULL.
*/
...
Node* DFS(Node* initial, int* cont){
  ...
  while (!is_empty(S)){
    Node* lectura = top(S);
    if (is_final(lectura)) return lectura;  // Se encontró la solución
    pop(S);
